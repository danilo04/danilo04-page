...
category: mobile
coverImage: /zoom-compose-cover.webp
...
<Video src="/media/sample-app-graphics-layer.mp4" />
...
![Image size versus container size](/zoom-image-size.webp)
...
We start the definition of the `Modifier.zoomableAwaitEachGesture` modifier as follows:
...
<div style={{ display: 'flex', justifyContent: 'center' }}>
  <img src="/media/touch-events-setup.GIF" alt="Touch events setup demonstration" width="400" style={{ display: 'block' }} />
</div>

<div style={{ paddingBottom: '2rem' }} />


Let us study the code step by step. First, we start by defining the `Modifier.zoomableAwaitEachGesture` modifier. This modifier is a modifier that applies the zoom effect to the image. The `ZoomableState` is a state object that contains the current zoom level and the minimum and maximum zoom levels. The `Modifier` is the modifier that is applied to the image.
