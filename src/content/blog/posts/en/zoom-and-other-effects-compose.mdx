...
This implementation still has some issues. When performing a panning gesture, we move outside the image. We need to limit the panning gesture to the image boundaries. To do this, we need to compute the maximum offset value based on the image size and the current scale value. To get the image size, we use `Modifier.onSizeChange` modifier to get the size of the image. 
...
Let us update the `computeOffset` method to use the `computeMaxOffset` method.
